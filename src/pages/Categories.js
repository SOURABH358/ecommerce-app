...
export default function Categories() {
    ...
    return <section className="w-full min-h-[100vh] bg-light-grey">
        ...
        <p className="py-2 px-4 bg-[white] mx-2 rounded-md mt-2">
            ...
            <span className="">{currentCat}</span>
        </p>
        <div className="w-full flex md:gap-x-4 mt-6 pb-12">
            <div className="md:w-[17rem] w-[13rem] h-fit top-0 bg-[white] py-4 px-4 sticky  rounded-r-lg">
                ...
                <div >
                    {categories.map((item, i) => {
                        ...
                        rounded-md ${item === currentCat ? 'bg-purple text-[white]' : null}`} onClick={() => setCurrentCat(item)}>{item}</p>
                    })}
                </div>
            </div>
                    ...
                    return item.categories === currentCat ?
                        <div key={item.id * 203} className="md:w-[22rem] w-full p-4 h-[30rem] rounded-xl shadow-2xl bg-[white] cursor-pointer"
                            ...
                            <Link to={`products/${item.id}`}><div className="w-full flex justify-center border-light-purple border-2 rounded-md py-4">
                                ...
                                <p className="text-left text-[0.9rem] mt-4 text-purple border-purple border-[1px] w-fit px-2 
                                rounded-md">Shopzy certified</p>
                            </Link>
                        </div> : null
                })}
            </div>
